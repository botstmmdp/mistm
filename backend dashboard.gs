...
function doGet(e) {
  try {
    ...
    // 1. Limpieza de registros pasados (Controlada por propiedad para no saturar cada carga)
    const scriptProps = PropertiesService.getScriptProperties();
    const lastClean = scriptProps.getProperty('LAST_CLEAN_DATE');
    const todayStr = Utilities.formatDate(new Date(), Session.getScriptTimeZone(), "yyyy-MM-dd");
    
    if (lastClean !== todayStr && (!e || !e.parameter || e.parameter.skipClean !== 'true')) {
      limpiarRegistrosViejos(ss);
      scriptProps.setProperty('LAST_CLEAN_DATE', todayStr);
    }

    // Capturamos parámetros
    let uFilter = (e && e.parameter && e.parameter.userId) ? e.parameter.userId.toString() : null;
    let sectionsParam = (e && e.parameter && e.parameter.sections) ? e.parameter.sections.toString().toUpperCase() : null;
    let sections = sectionsParam ? sectionsParam.split(',') : null;

    const result = { status: 'SUCCESS' };

    // Si no se especifican secciones, se envían todas (compatibilidad)
    const shouldInclude = (sec) => !sections || sections.indexOf(sec) !== -1;

    if (shouldInclude('CONVENIOS')) result.CONVENIOS = getSheetDataOptimized(ss, SHEETS.CONVENIOS);
    if (shouldInclude('NOVEDADES')) result.NOVEDADES = getSheetDataOptimized(ss, SHEETS.NOVEDADES);
    if (shouldInclude('USUARIOS')) result.USUARIOS = getSheetDataOptimized(ss, SHEETS.USUARIOS);
    if (shouldInclude('MENSAJES')) result.MENSAJES = getSheetDataOptimized(ss, SHEETS.MENSAJES, uFilter);
    if (shouldInclude('MENSAJES_RECIBIDOS')) result.MENSAJES_RECIBIDOS = getSheetDataOptimized(ss, SHEETS.MENSAJES_RECIBIDOS);
    if (shouldInclude('AFILIACIONES')) result.AFILIACIONES = getSheetDataOptimized(ss, SHEETS.AFILIACIONES);
    if (shouldInclude('FECHAS_QUINCHO')) result.FECHAS_QUINCHO = getSheetDataOptimized(ss, SHEETS.FECHAS_QUINCHO);
    if (shouldInclude('SOLICITUD_QUINCHO')) result.SOLICITUD_QUINCHO = getSheetDataOptimized(ss, SHEETS.SOLICITUD_QUINCHO);
    if (shouldInclude('CAPACITACIONES')) result.CAPACITACIONES = getSheetDataOptimized(ss, SHEETS.CAPACITACION);
...
function doPost(e) {
  try {
    ...
    const { action, sheet, id, data, userId } = payload;
    const reqData = payload.requestData || payload;
    const ss = SpreadsheetApp.openById(STM_MAIN_SPREADSHEET_ID);
    ...
    if (action === 'saveConsultation') {
      return saveConsultation(payload);
    }
    ...
    if (action === 'APPROVE_QUINCHO') {
      ...
      const bId = reqData.bookingId || id;
      const dId = reqData.dateId;

      // FORZAMOS LOS VALORES: 'NO' para pendiente y 'OCUPADO' para estado
      const updateSol = updateRecord(sSol, bId, payload.data || { pendiente: 'NO' });
      const updateFec = updateRecord(sFec, dId, { estado: 'OCUPADO' });
      ...
      if (userId && sMsg) {
        createRecord(sMsg, {
          ...
          mensaje: payload.message || reqData.message,
    ...
    if (action === 'CANCEL_QUINCHO') {
      ...
      // 1. Liberar la fecha PRIMERO para asegurar disponibilidad
      let resFecC = { status: 'SKIP' };
      if (dId && sFecC) {
        resFecC = updateRecord(sFecC, dId, { estado: 'DISPONIBLE' });
      }

      // 2. Eliminar la solicitud después
      const resDel = deleteRecord(sSolC, bId, userId);

      // NOTIFICACIÓN: Si hay userId, enviar mensaje de liberación
      if (userId && userId != 0) {
        const ss = sSolC.getParent();
        ...
        if (msgSheet) {
          msgSheet.appendRow([
            ...
            userId,
            "Reserva Liberada",
            "La administración ha liberado su reserva de quincho. Si lo desea, puede solicitar una nueva fecha.",
            new Date()
          ]);
        }
      }
    ...
    switch (action) {
      case 'CREATE':
        // REFUERZO: Forzar pendiente='SI' si la hoja es de solicitudes de quincho
        if (sheet === 'SOLICITUD_QUINCHO' || (targetSheet && targetSheet.getName().toLowerCase().includes('solicitud_quincho'))) {
          data.pendiente = 'SI';
        }
 ...
 */
/**
 * Versión optimizada que recibe el objeto Spreadsheet para evitar re-aperturas
 */
function getSheetDataOptimized(ss, sheetName, userIdFilter = null) {
...
function createRecord(sheet, data) {
  ...
  const newRow = headers.map(header => {
    ...
    const sheetNameNorm = (sheet.getName() || "").toLowerCase().trim();
    const targetSheetNorm = (SHEETS.SOLICITUD_QUINCHO || "").toLowerCase().trim();
    
    // FORZADO DE PENDIENTE = SI para nuevas solicitudes de quincho (Búsqueda ultra-flexible)
    const isQuinchoSheet = sheetNameNorm.includes('solicitud_quincho') || targetSheetNorm.includes('solicitud_quincho');
    if (key.includes('pendiente') && isQuinchoSheet) {
      return 'SI';
    }
  ...
  if (sheetName === SHEETS.USUARIOS.toLowerCase().trim() && data.password) {
    ...
    headers.forEach((header, index) => {
      ...
      if (key) {
        ...
        // Convertir fechas de Google a texto para el Dashboard
...
/**
 * LÓGICA DE LIMPIEZA OPTIMIZADA: Borra fechas anteriores a HOY y sus reservas
 * Usa setValues en lugar de deleteRow para mayor velocidad
 */
function limpiarRegistrosViejos(ss) {
  try {
    ...
    if (dataFec.length <= 1) return; // Solo encabezados
    ...
    let fechasBorradas = [];
    let nuevasFechas = [headersFec];
    
    // Filtrar fechas futuras
    for (let i = 1; i < dataFec.length; i++) {
      ...
      if (fechaObj < hoy) {
        ...
        nuevasFechas.push(dataFec[i]);
      }
    }

    // Actualizar hoja de fechas si hubo cambios
    if (nuevasFechas.length !== dataFec.length) {
      sFec.clearContents();
      sFec.getRange(1, 1, nuevasFechas.length, nuevasFechas[0].length).setValues(nuevasFechas);
    }

    // Borrar solicitudes asociadas de forma eficiente
    if (sSol && fechasBorradas.length > 0) {
      ...
      if (dataSol.length > 1) {
        ...
        if (colFechaSol !== -1) {
          let nuevasSol = [headersSol];
          for (let j = 1; j < dataSol.length; j++) {
            if (!fechasBorradas.includes(dataSol[j][colFechaSol].toString())) {
              nuevasSol.push(dataSol[j]);
            }
          }
          
          if (nuevasSol.length !== dataSol.length) {
            sSol.clearContents();
            sSol.getRange(1, 1, nuevasSol.length, nuevasSol[0].length).setValues(nuevasSol);
          }
        }
      }
    }
